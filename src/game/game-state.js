...
async function refreshFromServer() {
  if (userId && supabase) {
    const { data } = await supabase.from('toss_profiles').select('balance,claimed_goals').eq('id', userId).single();
    if (data) {
...
export async function setNickname(name) {
  ...
  if (userId) {
    try {
      await supabase.from('toss_profiles').update({ username: name }).eq('id', userId);
    } catch (e) {
...
export async function claimTossAdReward() {
  rolloverDayIfNeeded();
  // 보상 지급 + 일일 한도는 서버(toss_watch_ad)가 검증한다.
...
async function save() {
  ...
  try {
    ...
    const { error } = await supabase.from('toss_profiles').update({ game_state: data }).eq('id', userId);
...
export async function resetGame() {
  if (userId) {
    await supabase.from('toss_profiles').update({ game_state: {} }).eq('id', userId);
  } else {
