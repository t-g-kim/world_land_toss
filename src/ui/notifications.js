...
async function loadRecent() {
  if (!supabase) return;
  const { data } = await supabase.from('toss_notifications')
...
function subscribe() {
  ...
  channel = supabase.channel('my-notifications')
    .on('postgres_changes',
      { event: 'INSERT', schema: 'public', table: 'toss_notifications', filter: `user_id=eq.${uid}` },
      (payload) => {
        const n = payload.new;
...
async function markRead() {
  ...
  try { await supabase.rpc('toss_mark_notifications_read'); } catch { /* ignore */ }
}
